...
@Injectable()
export class EnatScraper extends BaseScraper {
  ...
  private readonly logger = new Logger(EnatScraper.name);

  constructor() { super(); }

  /**
   * Fetches Enat's exchange rate page via Playwright (hash-based navigation needs JS).
   */
  async scrapeRates(): Promise<RawScrapedRate[]> {
    try {
      ...
      try {
        ...
        // Wait for hash navigation to load exchange section
        await page.waitForTimeout(3000);
        const html = await page.content();
        await page.close();
        await context.close();

        return this.parseEnatRates(html);
      } finally {
        await browser.close();
      }
    } catch (error) {
      this.logger.error(`Enat Playwright scrape failed: ${error instanceof Error ? error.message : 'Unknown'}`);
      throw error;
    }
  }

  parseEnatRates(html: string): RawScrapedRate[] {
    const $ = cheerio.load(html);
    const rates: RawScrapedRate[] = [];

    // Enat has the exchange section loaded via hash navigation
